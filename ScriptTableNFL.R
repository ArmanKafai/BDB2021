...
Team <- read.csv("TeamScore.csv") #Team Score from the previous file
Defender<- read.csv("DefenderScore.csv") #Defender Score form the Previous File
Positions <- read.csv ("C:/Users/hound/OneDrive/Desktop/R/BDB/players.csv")

TeamPlay<- read.csv ("Defender.csv") #From the previous file
...
Total <- merge (POS, Defender, by.x='displayName', by.y='Defender') #Merging for Position
...
URL1<- CB1$team_wordmark #This saves the URL as a vector
